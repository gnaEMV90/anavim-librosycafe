export const siteContent = {
  brand: {
    name: 'ANAVIM',
    subtitle: 'Libros y Café',
    instagramUser: 'anavim.librosycafe',
    tagline: 'Donde cada página se disfruta con aroma a café.',
    description:
      'Una librería familiar que une buenas lecturas, café de especialidad y detalles preparados con intención: para regalar, compartir y disfrutar sin apuro.',
  },
  contact: {
    // Número real de WhatsApp, sin +, espacios ni guiones.
    whatsappNumber: '5493532678034',
    whatsappMessage:
      'Hola ANAVIM, quiero consultar por libros, café o promos disponibles.',
    instagramUrl: 'https://www.instagram.com/anavim.librosycafe/',
    email: '',
    location: 'Oliva, Córdoba',
    note:
      'La web funciona como showroom: consultás disponibilidad y coordinamos la compra directo por mensaje.',
  },
  hero: {
    eyebrow: 'Librería cristiana familiar & café de especialidad',
    title: 'Libros, café y regalos con propósito para disfrutar sin apuro.',
    text:
      'ANAVIM nace como un sueño familiar: acercar lecturas que edifiquen, café rico y detalles simples que inviten a compartir, regalar y volver a lo importante.',
    primaryCta: 'Quiero consultar',
    secondaryCta: 'Ver novedades en Instagram',
    tags: ['Biblias', 'Literatura cristiana', 'Café', 'Regalos con sentido'],
  },
  valueCards: [
    {
      title: 'Lecturas elegidas con intención',
      text: 'No se trata de llenar estantes por llenar. Elegimos libros que acompañen, edifiquen, inspiren y den ganas de volver a leer.',
    },
    {
      title: 'Café con aroma a pausa',
      text: 'Café de especialidad para cortar el ruido, compartir una charla o acompañar ese capítulo que pedía cinco minutos más.',
    },
    {
      title: 'Regalos con sentido',
      text: 'Combos, frases, señaladores y detalles preparados con estética cálida para regalar algo personal, no un trámite envuelto.',
    },
  ],
  featured: [
    {
      category: 'Libros',
      title: 'Lecturas para el alma',
      text: 'Biblias infantiles, libros cristianos, devocionales y lecturas seleccionadas para distintas etapas de la vida.',
      cta: 'Consultar títulos',
      imageSrc: '/images/libros.png',
      imageAlt: 'Mamá y niña leyendo una Biblia infantil en un ambiente cálido y familiar.',
    },
    {
      category: 'Café',
      title: 'Café de especialidad',
      text: 'Café en grano o molido, según disponibilidad. Una buena taza no arregla el mundo, pero ayuda bastante.',
      cta: 'Consultar café',
      imageSrc: '/images/cafe.png',
      imageAlt: 'Taza de café vista desde arriba sobre una mesa de madera.',
    },
    {
      category: 'Promos',
      title: 'Combos ANAVIM',
      text: 'Opciones para regalar: libro + café + detalle. Esta sección queda lista para sumar fotos de productos extras.',
      cta: 'Ver promos',
      imageSrc: '',
      imageAlt: '',
    },
  ],
  catalog: {
    eyebrow: 'Catálogo',
    title: 'Productos disponibles para consultar',
    text:
      'Cuando el catálogo esté cargado desde el panel administrador, acá van a aparecer libros, café, promos y regalos con consulta directa por WhatsApp.',
    emptyText:
      'El catálogo administrable ya está preparado. Los productos se van a mostrar cuando estén cargados desde el panel privado.',
  },
  categories: [
    'Libros cristianos',
    'Devocionales',
    'Biblias',
    'Familia y crianza',
    'Crecimiento personal',
    'Café de especialidad',
    'Regalos y combos',
  ],
  process: [
    {
      title: 'Elegís lo que te interesa',
      text: 'Mirás los destacados, categorías o promos disponibles. Si algo te llama la atención, nos escribís.',
    },
    {
      title: 'Consultás por WhatsApp',
      text: 'Te confirmamos disponibilidad, opciones de café, libros relacionados y detalles para armar el pedido.',
    },
    {
      title: 'Coordinamos la entrega',
      text: 'Definimos forma de pago, retiro o envío según corresponda. Simple, claro y sin vueltas raras.',
    },
  ],
  gallery: [
    {
      tag: 'Recomendado',
      type: 'Reel',
      title: 'Literatura cristiana',
      text: 'Biblias, devocionales y literatura cristiana para grandes y chicos, con opciones para regalar o acompañar la lectura diaria.',
      labels: ['Biblias', 'Devocionales', 'Regalo'],
      imageSrc: '/images/instagram/literatura-cristiana.png',
      imageAlt: 'Publicación de ANAVIM sobre literatura cristiana, Biblias y devocionales.',
      url: 'https://www.instagram.com/reel/DORLhvmjpOv/?utm_source=ig_web_copy_link&igsh=MzRlODBiNWFlZA==',
    },
    {
      tag: 'Café',
      type: 'Reel',
      title: 'Café de especialidad',
      text: 'Una selección de café para acompañar lecturas, charlas y momentos de pausa. Colombia, Etiopía y Tanzania según disponibilidad.',
      labels: ['Colombia', 'Etiopía', 'Tanzania'],
      imageSrc: '/images/instagram/cafe-especialidad.png',
      imageAlt: 'Publicación de ANAVIM sobre café de especialidad.',
      url: 'https://www.instagram.com/reel/DRiP81_Dkv6/?utm_source=ig_web_copy_link&igsh=MzRlODBiNWFlZA==',
    },
    {
      tag: 'Niños',
      type: 'Reel',
      title: 'Material para niños',
      text: 'Recursos infantiles, rompecabezas y materiales didácticos para aprender, jugar y compartir en familia.',
      labels: ['Niños', 'Didácticos'],
      imageSrc: '/images/instagram/material-ninos.png',
      imageAlt: 'Publicación de ANAVIM sobre materiales y recursos para niños.',
      url: 'https://www.instagram.com/reel/DOer3Twjupx/?utm_source=ig_web_copy_link&igsh=MzRlODBiNWFlZA==',
    },
    {
      tag: 'Producto propio',
      type: 'Reel',
      title: 'Guardianes de la Fe',
      text: 'Material propio de ANAVIM para niños: devocionales, actividades y recursos para recortar, aprender y crecer en la fe.',
      labels: ['Niños', 'Devocionales'],
      imageSrc: '/images/instagram/guardianes-fe.png',
      imageAlt: 'Publicación de ANAVIM sobre Guardianes de la Fe.',
      url: 'https://www.instagram.com/reel/DQ-JbD5jhdJ/?utm_source=ig_web_copy_link&igsh=MzRlODBiNWFlZA==',
    },
  ],
  about: {
    title: 'Un sueño familiar, nacido de la fe y de una buena mesa.',
    paragraphs: [
      'El 2 de septiembre de 2025 hicimos público el sueño que veníamos pensando como familia y dimos el primer paso de este proyecto.',
      'ANAVIM nace de nuestra pasión por el Señor Jesús, por la fe y, por supuesto, por los libros y un buen café.',
      'En ANAVIM – Libros y Café vas a encontrar Biblias, literatura cristiana, libros para grandes y chicos, y café para acompañar cada lectura.',
      'Nuestro deseo es que cada página y cada taza sean un espacio de inspiración, encuentro y esperanza. Bienvenidos a este viaje, nos alegra que seas parte.',
    ],
    highlights: [
      {
        label: 'Inicio',
        title: '2 de septiembre de 2025',
        text: 'El día en que compartimos públicamente el sueño que veníamos gestando como familia.',
      },
      {
        label: 'Raíz',
        title: 'Fe, libros y café',
        text: 'Un proyecto nacido del amor por Jesús, las buenas lecturas y los encuentros simples alrededor de una mesa.',
      },
      {
        label: 'Deseo',
        title: 'Inspiración y esperanza',
        text: 'Que cada página y cada taza abran un espacio de encuentro, descanso y propósito.',
      },
    ],
  },
  finalCta: {
    title: '¿Querés consultar disponibilidad o armar un regalo?',
    text:
      'Escribinos por WhatsApp y te ayudamos a elegir libros, café o combos según la ocasión. La idea es que sea simple, cálido y bien acompañado.',
  },
}
